...
for session_id = 1:config.session_num
    ...
    if session_id == 1
        ...
        eval(sprintf('labelname = [''A0%dTlabel.mat''];',sub_id));
    else
        eval(sprintf('filename = [''A0%dE.gdf''];',sub_id));
        eval(sprintf('labelname = [''A0%dElabel.mat''];',sub_id));           
    end
    ...
    f_s = zeros(size(s));
    for i = 1:size(s,2)
        f_s(:,i) = filtfilt(config.d, s(:,i));       %Butterworth filtering
    end
    
    % split continuous signals into epochs
